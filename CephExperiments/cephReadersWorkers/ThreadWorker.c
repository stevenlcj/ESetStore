...
void startThreadMgr(int startIdx, int endIdx, int threadNum){
    ...
    char cmdSuffix[]=" file\0";
    size_t suffixLen = strlen(cmdSuffix);
    size_t cmdStrLen = strlen(cmdStr);
    size_t cmdLen = cmdStrLen + suffixLen + 7;
    ...
    for (idx = startIdx; idx <= endIdx; ++idx) {
        ...
        memcpy(curCmd + curCmdLen, cmdSuffix, suffixLen);
        curCmdLen = curCmdLen + suffixLen;
        //printf("curCmdLen:%d\n",curCmdLen);
        curCmdLen = curCmdLen + numToStr(curCmd + curCmdLen , idx);
        *(curCmd + curCmdLen) = '\0';
