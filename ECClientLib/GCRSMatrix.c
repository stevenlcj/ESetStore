    ...
    for (columnIdx = 0; columnIdx < (k*w); ++columnIdx) {
        ...
        for (rowIdx = 0; rowIdx < (m*w); ++rowIdx) {
            ...
            if (rowIdx % w == 0) {
                ...
                if ((bitIdx + w) > (int)sizeof(int) * 8) {
                    bitIdx = 0;
                    intbitmatrixIdx = intbitmatrixIdx + 1;
                }
            }
            
            if (bitIdx >= (int) sizeof(int) * 8) {
                bitIdx = 0;
                intbitmatrixIdx = intbitmatrixIdx + 1;
            }
...
int grs_create_decoding_data_matrix(int k, int m, int w,
                                        int *matrix, int *decoding_data_matrix,
                                    int *erased, int *dm_ids){
    int i, j, *tmpmat;
    ...
    i = jerasure_invert_matrix(tmpmat, decoding_data_matrix, k, w);
    free(tmpmat);
    return i;
}
...
int* grs_create_decoding_matrix(int k, int m, int w,
                                int *matrix, int *mat_idx,
                                int *erasures, int *dm_ids){
    int kIdx, mIdx, matIdx;
    int *erases;
    ...
    decoding_matrix = talloc(int, (k * m));

    if ((erases = grs_erasures_to_erased(k, m, erasures)) == NULL) {
        return NULL;
    }
    
    if ((decoding_data_matrix =  talloc(int, k*k)) == NULL ) {
        return NULL;
    }
    ...
    if (dFailedNum > 0) {
        if(grs_create_decoding_data_matrix(k, m, w,
                                               matrix, decoding_data_matrix,
                                               erases, dm_ids) < 0){
            free(erases);
            free(decoding_data_matrix);
    ...
    for (kIdx = 0; kIdx < k; ++kIdx) {
        ...
        if (erases[kIdx] == 1) {
            *(mat_idx + matIdx) = kIdx;
            memcpy((decoding_matrix + matIdx * k), decoding_data_matrix + kIdx * k, sizeof(int) * k);
            matIdx = matIdx + 1;
        }
    }
    ...
    for (mIdx = 0; mIdx < m; ++mIdx) {
        if (erases[mIdx + kIdx] == 1) {
            *(mat_idx + matIdx) = mIdx + k;
            //Generate the vector for restoring
            memset((decoding_matrix + matIdx * k * w * w), 0, sizeof(int) * k * w * w);
            grs_generate_coding_vector(k,m,w,mIdx,
                                        matrix, decoding_data_matrix, (decoding_matrix + matIdx * k),
                                        erases);
            matIdx = matIdx + 1;
        }
    }
...
int *grs_erasures_to_erased(int k, int m,
                             int *erasures){
    int td;
    int t_non_erased;
    int *erased;
...
int grs_generate_coding_vector(int k, int m, int w, int mIdx,
                                int *matrix, int *invert_matrix, int *vector,
                               int *erases){
    return 0;
}
