...
int getMatrixValues(const char *str1, const char *str2, const char *str, int *matrix, size_t matSize){
    ...
    int idx = 0, idx1;
    char strBuf[1024], valueBuf[1024];
    ...
    memcpy(strBuf, (str + idx + str1Len), (idx1 - (idx + str1Len)));
    strBuf[(idx1 - ((idx + str1Len)))] = '\0';
    ...
    char spaceCh = ' ';
    for (idx = 0; idx < (int) matSize; ++idx) {
        while (*(strBuf+startOffset) == spaceCh) {
            ++startOffset;
        }
        
        endOffset = startOffset + 1;
        
        while (*(strBuf+endOffset) != spaceCh) {
            
            if (*(strBuf+endOffset) == '\0') {
                return -1;
            }
            ++endOffset;
        }
        
        //        printf("startOffset:%lu, endOffset:%lu\n", startOffset, endOffset);
        
        memcpy(valueBuf, strBuf+startOffset, (endOffset - startOffset));
        valueBuf[(endOffset - startOffset)] = '\0';
        *(matrix+idx) = str_to_int(valueBuf);
...
//        printf("startOffset:%lu, endOffset:%lu\n", startOffset, endOffset);
        ...
        memcpy(valueBuf, strBuf+startOffset, (endOffset - startOffset));
        valueBuf[(endOffset - startOffset)] = '\0';
        *(bitmatrix+idx) = str_to_int(valueBuf);
...
size_t sizeT_to_str(size_t value, char *buf, size_t bufSize){
    ...
    size_t strSize, digitInDeci = 1;
    size_t divideDeci = 10, valueToDiv = value;
...
void copyStrToBufOffset(char *desBuf, char *srcBuf, size_t *writeOffset){
    size_t writeSize = strlen(srcBuf);
    memcpy((desBuf + *writeOffset), srcBuf, writeSize);
    *writeOffset = *writeOffset + writeSize;
}
...
size_t alignBufSize(size_t bufSizeToAlign, size_t alignValue){
    if (alignValue == 0){
        return bufSizeToAlign;
    }

    if (bufSizeToAlign < alignValue)
    {
        bufSizeToAlign = alignValue;
    }

    if ((bufSizeToAlign % alignValue) == 0 )
    {
        return bufSizeToAlign;
    }

    size_t alignedValue = alignValue - (bufSizeToAlign % alignValue);

    return (bufSizeToAlign + alignedValue);
}
