                             ...
                             char **data_ptrs, char **coding_ptrs, int size, int packetsize)
{
  ...
  if (size%(w*packetsize) != 0) {
    fprintf(stderr, "jerasure_bitmatrix_dotprod - size mod (w*packetsize)) must = 0\n");
    exit(1);
  }
...
void jerasure_do_scheduled_operations2(char **ptrs, int **operations, int packetsize, int size, int w)
{
  char *sptr;
  char *dptr;
  int op;
  ...
  for (tdone = 0; tdone < size; tdone += packetsize*w) {
	  for (op = 0; operations[op][0] >= 0; op++) {
		sptr = ptrs[operations[op][0]] + i*w*packetsize + operations[op][1]*packetsize;
		dptr = ptrs[operations[op][2]] + i*w*packetsize + operations[op][3]*packetsize;
		if (operations[op][4]) {
	/*      printf("%d,%d %d,%d\n", operations[op][0], 
		  operations[op][1], 
		  operations[op][2], 
                                   ...
                                   char **data_ptrs, char **coding_ptrs, int size, int packetsize)
{
  ...
  jerasure_do_scheduled_operations2(ptr_copy, schedule, packetsize, size, w);
  free(ptr_copy);
}
                            ...
                            char **data_ptrs, char **coding_ptrs, int size, int packetsize)
{
  ...
  if (packetsize%sizeof(long) != 0) {
    fprintf(stderr, "jerasure_bitmatrix_encode - packetsize(%d) mod sizeof(long) != 0\n", packetsize);
    exit(1);
  }
  if (size%(packetsize*w) != 0) {
    fprintf(stderr, "jerasure_bitmatrix_encode - size(%d) mod (packetsize(%d)*w(%d))) != 0\n",
         size, packetsize, w);
    exit(1);
  }
