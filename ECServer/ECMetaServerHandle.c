...
void processRecoveryNodes(ECBlockServerEngine_t *ecBlockServerEnginePtr){
    ...
    if (ecBlockServerEnginePtr->recoveryThMgr == NULL) {
        ecBlockServerEnginePtr->recoveryThMgr = createRecoveryThreadMgr(ecBlockServerEnginePtr->readMsgBuf->buf, (void*) ecBlockServerEnginePtr);
        writeRecoveryNodesOKReplyCmd(ecBlockServerEnginePtr->recoveryThMgr->eSetIdx , (void *)ecBlockServerEnginePtr);
    }else{
...
void processRecoveryNodesOver(ECBlockServerEngine_t *ecBlockServerEnginePtr){
    ...
    if (ecBlockServerEnginePtr->recoveryThMgr->eSetIdx != setIdx) {
        printf("Error: ecBlockServerEnginePtr->ecRecoveryHandlePtr->eSetIdx:%d != setIdx:%d\n",ecBlockServerEnginePtr->recoveryThMgr->eSetIdx, setIdx);
        return;
    }
    
    deallocRecoveryThreadMgr(ecBlockServerEnginePtr->recoveryThMgr);
...
//            printf("Recvd heartbeat\n");
        ...
        case MetaServerCommand_RecoverBlock:{
            if (ecBlockServerEnginePtr->recoveryThMgr == NULL) {
                printf("Error: ecBlockServerEnginePtr->recoveryThMgr = NULL\n");
            }
            
            if(initRecoveryTask(ecBlockServerEnginePtr->recoveryThMgr, ecBlockServerEnginePtr->readMsgBuf->buf) != 0){
...
//        if (ecBlockServerEnginePtr->ecRecoveryHandlePtr != NULL ) {
//            if (ecBlockServerEnginePtr->ecRecoveryHandlePtr->blockGroupIdx != 0 && ecBlockServerEnginePtr->ecRecoveryHandlePtr->curBlockDone == 1) {
//                char* cmd = formRecoveryBlockOverCmd(ecBlockServerEnginePtr->ecRecoveryHandlePtr->eSetIdx, ecBlockServerEnginePtr->ecRecoveryHandlePtr->blockGroupIdx);
//                addWriteMsgToMetaServer(ecBlockServerEnginePtr, cmd, strlen(cmd) + 1);
//                processMetaOutMsg(ecBlockServerEnginePtr);
//                free(cmd);
//                
//                ecBlockServerEnginePtr->ecRecoveryHandlePtr->blockGroupIdx = 0;
//            }
//        }
        ...
        for (idx = 0 ; idx < eventsNum; ++idx) {
               ...
               (!(events[idx].events & EPOLLIN) && !(events[idx].events & EPOLLOUT)))
            {
                ...
                if (ecBlockServerEnginePtr->recoveryThMgr->blockGroupIdx != 0 && ecBlockServerEnginePtr->recoveryThMgr->curBlockDone == 1) {
                    writeRecoveryBlockOverCmd(ecBlockServerEnginePtr->recoveryThMgr->eSetIdx, ecBlockServerEnginePtr->recoveryThMgr->blockGroupIdx, (void *)ecBlockServerEnginePtr);
                    processMetaOutMsg(ecBlockServerEnginePtr);
                    
                    ecBlockServerEnginePtr->recoveryThMgr->blockGroupIdx = 0;
                }
